...
function pagination(totalPosts) {
  ...
  if (currentPage === 1 || maximum <= 1) return;
  ...
  const numeracion = document.getElementById("numeracion-paginacion");
  if (numeracion) {
    numeracion.innerHTML = `<div style="text-align:center;">${paginationHTML}</div>`;
  }
}
