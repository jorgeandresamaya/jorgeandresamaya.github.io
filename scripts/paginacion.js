...
function pagination(totalPosts) {
    ...
    if (maximum <= 1) return; // Si solo hay una página, no mostrar numeración
    ...
    // Insertar HTML en contenedor específico
    let pagerElement = document.getElementById("numeracion-paginacion");
    if (pagerElement) pagerElement.innerHTML = paginationHTML;

    // También actualizar blog-pager para compatibilidad
    let blogPager = document.getElementById("blog-pager");
    if (blogPager) {
        let newerLink = blogPager.querySelector(".blog-pager-newer-link");
        let olderLink = blogPager.querySelector(".blog-pager-older-link");
        blogPager.innerHTML = "";
        if (newerLink) blogPager.appendChild(newerLink);
        blogPager.appendChild(pagerElement);
        if (olderLink) blogPager.appendChild(olderLink);
    }
}
