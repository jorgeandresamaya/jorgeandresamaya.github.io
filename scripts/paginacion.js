...
// var itemsPerPage = 10;
// var pagesToShow = 5;       // Ej. 5: [Anterior] ... 3 4 [5] 6 7 ... [Siguiente]
...
function renderPagination(totalPosts) {
    ...
    // TODO: Ajuste aquí
    // La numeración completa solo se genera si NO es la primera página Y hay más de una página en total.
    if (currentPage > 1 && maximum > 1) {
        ...
        // Lógica para mostrar solo 'pagesToShow' números (ej. 5)
        let delta = Math.floor(pagesToShow / 2);
        let start = currentPage - delta;
        let end = currentPage + delta;

        // Ajustar start y end para los extremos
        if (start < 1) {
            end += (1 - start);
            start = 1;
        }
        ...
        start = Math.max(1, start); // Asegurar que start no sea menor que 1

        // Siempre mostrar la primera página si no está incluida
        if (start > 1) {
            paginationHTML += createPageLink(1, "1");
            if (start > 2) paginationHTML += "<span class='pagenumber dots'>...</span>";
        }
        ...
        // Siempre mostrar la última página si no está incluida
        if (end < maximum) {
            if (end < maximum - 1) paginationHTML += "<span class='pagenumber dots'>...</span>";
        ...
        console.log("No custom pagination displayed: currentPage is 1 or maximum pages is 1. Showing default Blogger pager only.");
    }
    ...
    if (pagerElement) {
        ...
        if (paginationHTML) {
            ...
            // Intento 4: Si no hay botones predeterminados, pero sí hay #blog-pager, y no estamos en Home, reemplazar su contenido
            // (Si estamos en Home, no queremos poner nada)
            else {
                console.log("No specific prev/next buttons found. Replacing #blog-pager content.");
                pagerElement.innerHTML = paginationHTML;
            }
        } else {
            console.log("No custom pagination HTML generated for current page conditions. Ensuring no custom wrapper is left.");
            // Si no hay paginación para mostrar, nos aseguramos de que el contenedor esté vacío o inexistente
        }
    } else {
