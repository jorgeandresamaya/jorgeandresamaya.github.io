...
function pagination(totalPosts) {
    ...
    if (maximum <= 1) return; // Solo mostrar si hay más de 1 página
    ...
    // Numeración solo a partir de la página 2
    if (currentPage >= 2) {
    ...
    // Insertar en el contenedor de numeración y centrar
    let numeracion = document.getElementById("numeracion-paginacion");
    if (numeracion) {
        numeracion.innerHTML = `<div style="display:flex;justify-content:center;flex-wrap:wrap;">${paginationHTML}</div>`;
    }
}
