...
function pagination(totalPosts) {
    ...
    if (maximum <= 1) return; // No mostrar si solo hay 1 página
    ...
    // Insertar numeración centrada
    var numeracion = document.getElementById("numeracion-paginacion");
    if (numeracion) {
        numeracion.innerHTML = `<div style="display:flex;justify-content:center;flex-wrap:wrap;gap:5px;">${paginationHTML}</div>`;
    }
}
